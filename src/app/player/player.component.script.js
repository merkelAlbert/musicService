...
function queueAudio() {
  ...
  let track = tracks.indexOf(audio.src) + 1;
...
function newSong() {
  if (preloaded) {
    let parentEl = audio.parentNode;
    let newTrack = tracks.indexOf(audioPreload.src);
    $('#playlist li').removeClass('playing');
...
let play = function() {
  ...
  let index = tracks.indexOf(audio.src);
...
$('#next').click(function() {
  let track = tracks.indexOf(audio.src);
  if (track == -1) {
    // WAT
